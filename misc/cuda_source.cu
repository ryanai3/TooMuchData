...
#define CUPY_FOR(i, n) \
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; \
         i < (n); \
         i += blockDim.x * gridDim.x)
    extern "C" __global__ void binary_hierarchical_softmax_forward(const CArray<float, 2> x, const CArray<float, 2> w, const CArray<int, 1> ts, const CArray<int, 1> paths, const CArray<float, 1> codes, const CArray<int, 1> begins, const int c, const int max_length, CArray<float, 1> _raw_ls, CArray<float, 1> _raw_wxy, CIndexer<1> _ind) {
      ;
      CUPY_FOR(i, _ind.size()) {
        _ind.set(i);
        T &ls = _raw_ls[_ind.get()];
T &wxy = _raw_wxy[_ind.get()];
...
#define CUPY_FOR(i, n) \
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; \
         ...
         i += blockDim.x * gridDim.x)
extern "C" 
__global__ void binary_hierarchical_softmax_forward(
  const CArray<float, 2> x, 
  const CArray<float, 2> w, 
  const CArray<int, 1> ts, 
  const CArray<int, 1> paths, 
  const CArray<float, 1> codes, 
  const CArray<int, 1> begins, 
  const int c, 
  const int max_length, 
  CArray<float, 1> _raw_ls, 
  CArray<float, 1> _raw_wxy, 
  CIndexer<1> _ind
) {
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < _ind.size(); i += blockDim.x * gridDim.x) {
    _ind.set(i);
    T &ls = _raw_ls[_ind.get()];
    T &wxy = _raw_wxy[_ind.get()];
    int ind = i / max_length;
