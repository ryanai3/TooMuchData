...
def train_epoch(model, optimizer, data_iter, desc, data_iter_len, bptt_len):#, dev_iter):
  p_bar = tqdm(
    iterable = data_iter,
    desc = desc,
    total = data_iter_len,
...
#  for batch in p_bar:
#    try:
#      if i % (10 * 1000) == 0 and i != 0:
#        dev_epoch(model, islice(dev_iter, 10 * 1000), desc + "_minidev", 10 * 1000)
#        model.init_state(train = True)
#        model.zero_grad()
    if i % bptt_len == 0 and i != 0:
      ...
      p_bar.set_postfix(ls=loss.data[0], ls_ep = ep_loss_norm, px = 2 ** loss.data[0], px_ep = 2 ** ep_loss_norm, ls_100 = curr_ce_100, px_100 = 2 ** curr_ce_100)
      loss = 0.0
    loss += model(batch)
    del batch
    i += 1
