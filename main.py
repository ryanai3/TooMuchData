...
#from chainer.serializers import NP as Serializer
from chainer.serializers import load_npz as load_model
from chainer.serializers import save_npz as save_model
from chainer.iterators import MultiprocessIterator as MPIterator
...
def get_huffman_tree(params):
  if "huff_tree_loc" in params:
    ...
    params["vocab_counts"] = all_counts #TODO: REMOVE AFTER DONE WITH BLACKOUT
...
#  n_words_dev = 573618777
  ...
  if epoch_num == 1 and False: # first epoch!
    dev_epoch(
      model = model,
      data_iter = dev_iter,
...
def train_epoch(model, optimizer, data_iter, desc, data_iter_len, bptt_len):
  ...
  for batch in p_bar:
    try:
      if counter % bptt_len == 0 and counter != 0:
        loss = loss / bptt_len
        loss.backward()
        optimizer.update()
        model.stop_bptt()
        model.cleargrads()
        p_bar.set_postfix(loss=loss.data, pplx = 2 ** loss.data)
        loss = 0.0
      loss += model(batch)
      counter += 1
...
def dev_epoch(model, data_iter, desc, data_iter_len):
  ...
  for i, batch in enumerate(p_bar):
    try:
      ...
      p_bar.set_postfix(loss=loss/(i + 1))
    except Exception as exc:
      import pdb; pdb.set_trace()
      print("Error")   
  print("")
