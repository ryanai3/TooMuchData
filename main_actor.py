...
#  for batch in p_bar:
    try:
      if i % bptt_len == 0 and i != 0:
        ...
        dloss = loss.data[0]
        ce_100[i % 100] = dloss
        curr_ce_100 = ce_100.sum()/min(i, 100)
        epoch_loss += dloss
        ep_loss_norm = epoch_loss/(i/bptt_len)
        p_bar.set_postfix(ls=loss.data[0], ep_ls = ep_loss_norm, px = 2 ** loss.data[0], ep_px = 2 ** ep_loss_norm, ls_100 = curr_ce_100, px_100 = 2 ** curr_ce_100)
        loss = 0.0
      loss += model(batch)
      i += 1
