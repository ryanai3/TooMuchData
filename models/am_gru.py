...
class ActorGRU(nn.Module):
  ...
  def forward(self, batch):
    ...
    am_idY = Variable(am_idY.clamp(-1, self.actor_cast_size - 1))
    wv = self.embed(wX)
    ...
    bn_h = self.bn(h)
    attn_read, mention_scores = self.actor_pool.query(h)
    mention_loss = 0
    if am_locY.dim() > 0:
      mention_loss = F.cross_entropy(mention_scores[am_locY], am_idY[am_locY])
    self.state = h
    if story_end.dim() > 0:
      self.end_stories(story_end)
    late_fuse = self.dropout(torch.cat((bn_h, attn_read), 1))
    loss = self.hsm(late_fuse, wY.data)/self.batch_size
    return (loss, mention_loss)
