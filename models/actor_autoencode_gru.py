...
class ActorGRU(nn.Module):
  ...
  def __init__(self, params):
    ...
    self.emotions = nn.Parameter(
      torch.zeros(self.n_emotions, self.word_dim)
    )
    self.emotion_dist = nn.Linear(
      in_features = self.actor_n_units,
      out_features = self.n_emotions
    )
  ...
  def stop_bptt(self):
    self.state = Variable(self.state.data, requires_grad=False)
#    self.state.detach_()
  ...
  def __call__(self, batch):
    ...
    if sentence_vecs is not None:
      ...
      em_dist = self.emotion_dist(actor_h)
      weighted = em_dist @ self.emotions
      reconstruction_loss = (sentence_vecs - weighted).squeeze().abs().norm(p=1)
